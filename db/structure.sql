...
CREATE FUNCTION users_update_searchable_column() RETURNS trigger
    ...
    AS $$
BEGIN
   NEW.searchable = COALESCE(NEW.lastname::text, '') || ' ' || COALESCE(NEW.firstname::text, '') || ' ' || COALESCE(NEW.email::text, '') || ' ' || COALESCE(NEW.login::text, '') || ' ' || COALESCE(NEW.org_id::text, '') ;
   RETURN NEW;
END;
$$;
...
CREATE UNIQUE INDEX users_org_id_idx ON users USING btree (org_id);
