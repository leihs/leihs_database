      ...
      dir.up do
        MigrationBuilding.all.group_by(&:name).each_pair do |name, buildings|
          if buildings.count == 1
            ...
            to_keep_building, *to_delete_buildings = buildings
            rooms = to_delete_buildings.map(&:rooms).flatten

            rooms.each do |room|
              target_room = to_keep_building.rooms.detect do |r|
                room.general? ? r.general? : r.name == room.name
              end

              if target_room
                room.items.update_all(room_id: target_room.id)
                room.procurement_requests.update_all(room_id: target_room.id)
              else
                room.update!(building_id: to_keep_building.id)
              end
            end

            to_delete_buildings.each(&:destroy!) # destroy the rooms too
          end
        end
